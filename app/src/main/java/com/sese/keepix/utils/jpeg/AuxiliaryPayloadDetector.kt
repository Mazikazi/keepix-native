 ...
 * alongside it, in an APP1 segment: Ultra HDR, Google's GContainer directory,
 * and both generations of Google/Samsung motion photos all self-describe with
 * a recognisable ASCII marker string in that XMP. This is a fail-closed
 * detector: any one of those markers found anywhere in an APP1 payload makes
 * the file ineligible for the MPF-stripping rewrite, full stop. It is
 * deliberately not trying to fully parse XMP/RDF -- a plain byte-substring
 * search is enough to recognise these markers, and parsing more than that
 * would be more code with no more safety.
 *
