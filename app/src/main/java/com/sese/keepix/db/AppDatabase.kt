...
abstract class AppDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        /**
         * Adds [BinItemEntity.trashed]. Existing rows default to 0, which is
         * the honest value: everything binned before this shipped was never
         * trashed, so those files are still visible in the gallery and still
         * need a trash request.
         */
        val MIGRATION_6_7 = object : Migration(6, 7) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL(
                    "ALTER TABLE bin_items ADD COLUMN trashed INTEGER NOT NULL DEFAULT 0"
                )
            }
        }
